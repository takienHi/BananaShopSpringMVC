...
(function() {
			...
			on: function(b, d, f, h, k) {
				...
				if (0 > g.getListenerIndex(d)) {
					...
					for (var A = g.length - 1; 0 <= A; A--)
						if (g[A].priority <= k) return g.splice(A + 1, 0, q), {
							removeListener: t
						};
					g.unshift(q)
				}
			...
			fire: function() {
				...
				return function(k, q, t) {
					...
					if (g) {
						var A = g.listeners;
						if (A.length)
							for (var A = A.slice(0), v, C = 0; C < A.length; C++) {
								if (g.errorProof) try {
									v =
										A[C].call(this, t, q, b, h)
								} catch (n) {} else v = A[C].call(this, t, q, b, h);
	...
	CKEDITOR.on("log", function(a) {
		if (window.console && window.console.log) {
			var d = console[a.data.type] ? a.data.type : "log",
				b = a.data.errorCode;
			if (a = a.data.additionalData) console[d]("[CKEDITOR] " + b, a);
			else console[d]("[CKEDITOR] " + b);
	...
	(function() {
		...
		CKEDITOR.tools = {
			...
			buildStyleHtml: function(a) {
				...
				for (var g,
						b = [], c = 0; c < a.length; c++)
					if (g = a[c]) /@import|[{}]/.test(g) ? b.push("\x3cstyle\x3e" + g + "\x3c/style\x3e") : b.push('\x3clink type\x3d"text/css" rel\x3dstylesheet href\x3d"' + g + '"\x3e');
				return b.join("")
			},
	...
	CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
		...
		getIndex: function(a) {
			...
			do
				if (!a || b == this.$ || b.nodeType != CKEDITOR.NODE_TEXT || !e && b.nodeValue) c++, e = b.nodeType == CKEDITOR.NODE_TEXT; while (b = b.previousSibling);
			return c
		},
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			removeStyle: function(a) {
				...
				if (b.removeProperty || "border" != a && "margin" != a && "padding" != a) b.removeProperty ? b.removeProperty(a) : b.removeAttribute(CKEDITOR.tools.cssStyleToDomStyle(a)), this.$.style.cssText || this.removeAttribute("style");
					...
					for (var b = [], d = 0; d < c.length; d++)
						if (g)
							for (var A = 0; A < g.length; A++) b.push([a, c[d], g[A]].join("-"));
			...
			getDocumentPosition: function(a) {
				var b = 0,
					...
					g = this.getDocument(),
					d = g.getBody(),
					A = "BackCompat" == g.$.compatMode;
				if (document.documentElement.getBoundingClientRect) {
					var e = this.$.getBoundingClientRect(),
						f = g.$.documentElement,
						n = f.clientTop ||
						d.$.clientTop || 0,
						y = f.clientLeft || d.$.clientLeft || 0,
						h = !0;
					...
					for (n = this, y = null; n && "body" != n.getName() && "html" != n.getName();) {
			...
			copyAttributes: function(a,
				...
				for (var g = 0; g < c.length; g++) {
					var d = c[g],
						...
						v;
					if (!(e in b))
						if ("checked" == e && (v = this.getAttribute(e))) a.setAttribute(e, v);
						else if (!CKEDITOR.env.ie || this.hasAttribute(e)) v = this.getAttribute(e), null === v && (v = d.nodeValue), a.setAttribute(e, v)
				}
		...
		function a(a, b) {
			...
			if (!a && !this._.guardLTR) {
				var D = g.type == CKEDITOR.NODE_ELEMENT ? g : g.getParent(),
					r = g.type == CKEDITOR.NODE_ELEMENT ? g.getChild(z) : g.getNext();
	...
	(function() {
		...
		function d(a, b, c, d, e) {
			...
			function f() {
				...
				for (a = 0; a < g; a++)
					if (b = x[a], c = H[a], !b.equals(c)) return a;
				return a - 1
			}
			...
			var p = a.startContainer,
				...
				r = a.endOffset,
				G, B, J, t, q, S;
			if (l && m.type == CKEDITOR.NODE_TEXT && p.equals(m)) p = a.document.createText(p.substring(D, r)), c.append(p);
			else {
				...
				O = c;
				for (P = I; P <= K; P++)
					if (c = H[P], W = c.getPrevious(), c.equals(x[P])) b && (O = O.getChild(0));
					else {
		...
		CKEDITOR.dom.range.prototype = {
			...
			createBookmark2: function() {
				function a(c) {
					...
					if (g.type == CKEDITOR.NODE_TEXT) {
						e = g;
						for (f = 0;
							(e = e.getPrevious()) && e.type == CKEDITOR.NODE_TEXT;) f += e.getLength();
						e = f;
				...
				switch (a) {
					...
					case CKEDITOR.ENLARGE_ELEMENT:
						...
						for (; l || p;) {
							...
							for (; p;) {
								r = !1;
								if (p.type == CKEDITOR.NODE_TEXT) G = p.getText(), n(p, 0) || (p = null), r = /^[\s\ufeff]/.test(G);
					...
					case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
						...
						var M, S, x = CKEDITOR.dom.walker.blockBoundary(a == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS ? {
								br: 1
							} : null),
							H = null,
							I = function(a) {
			...
			moveToClosestEditablePosition: function(a, b) {
				var c, d = 0,
					e, n, f = [CKEDITOR.POSITION_AFTER_END, CKEDITOR.POSITION_BEFORE_START];
				a ? (c = new CKEDITOR.dom.range(this.root), c.moveToPosition(a, f[b ? 0 : 1])) : c = this.clone();
				if (a && !a.is(CKEDITOR.dtd.$block)) d = 1;
				else if (e = c[b ? "getNextEditableNode" : "getPreviousEditableNode"]()) d = 1, (n = e.type ==
	...
	(function() {
		...
		function b(a, c, e, f) {
			...
			if ((h = CKEDITOR.filter.instances[f.element.data("cke-filter")]) && !h.check(c)) return b(a,
				c, e, f.remaining);c = new CKEDITOR.dom.range(f.element);c.selectNodeContents(f.element);c = c.createIterator();c.enlargeBr = a.enlargeBr;c.enforceRealBlocks = a.enforceRealBlocks;c.activeFilter = c.filter = h;a._.nestedEditable = {
				element: f.element,
				container: e,
		...
		a.prototype = {
			getNextParagraph: function(a) {
				...
				if (!this._.started) {
					...
					h = n.startPath();
					var y = n.endPath(),
						F = !n.collapsed && c(n, h.block),
						w = !n.collapsed && c(n, y.block, 1);
				...
				for (this._.nextNode = null; y;) {
					var F = 0,
						w = y.hasAscendant("pre"),
						u = y.type != CKEDITOR.NODE_ELEMENT,
						l = 0;
					if (u) y.type == CKEDITOR.NODE_TEXT && e.test(y.getText()) && (u = 0);
					else {
						var p = y.getName();
						if (CKEDITOR.dtd.$block[p] && "false" == y.getAttribute("contenteditable")) {
							...
							b(this, a, d);
							break
						} else if (y.isBlockBoundary(this.forceBrBreak && !w && {
								br: 1
							})) {
							if ("br" == p) u = 1;
	...
	(function() {
		function a(a, b, c, d, l) {
			...
			for (d = a.length; c < d; ++c) {
				...
				if (!0 === e.elements ||
					null === e.elements) l[p ? "unshift" : "push"](e);
				else
					for (k in m = e.elements, delete e.elements, m)
						if (b[k]) b[k][p ? "unshift" : "push"](e);
						else b[k] = [e]
			}
		}
		...
		function c(a, b, c) {
			if (!a.match || a.match(b)) {
				...
				b = b.classes;
				if (a) {
					for (var d = !1, l = !0 === a, p = b.length; p--;)
						if (l || a(b[p])) b.splice(p, 1), d = !0;
					a = d
				} else a = !1;
		...
		function v(a, b, e, l) {
			...
			if (l.doCallbacks && a.elementCallbacks) {
				a: {
					...
					for (var u = 0, f = r.length, D; u < f; ++u)
						if (D = r[u](b)) {
							r = D;
							break a
						} r = void 0
			...
			if (l.doFilter) {
				a: {
					...
					if (a || r) {
						A(b);
						if (u)
							for (x = 0, h = u.length; x < h; ++x)
								if (!1 === c(u[x], b, g)) {
									a = null;
									break a
								} if (f)
							for (x = 0, h = f.length; x < h; ++x) c(f[x], b, g);
						if (a)
							for (x = 0, h = a.length; x < h; ++x) d(a[x], b, g, D);
						if (r)
							for (x = 0, h = r.length; x < h; ++x) d(r[x], b, g, D);
						a = g
					} else a = null
				}
				if (!a || !a.valid) return e.push(b),
				1;h = a.validAttributes;
				var G = a.validStyles;
				r = a.validClasses;
				var u = b.attributes,
					k = b.styles,
					f = b.classes;D = b.classBackup;
				var I = b.styleBackup,
					B, H, O = [],
					g = [],
					J = /^data-cke-/;x = !1;delete u.style;delete u["class"];delete b.classBackup;delete b.styleBackup;
				if (!a.allAttributes)
					for (B in u) h[B] || (J.test(B) ? B == (H = B.replace(/^data-cke-saved-/, "")) || h[H] || (delete u[B], x = !0) : (delete u[B], x = !0));
				...
				} else D && (u["class"] = D);x && (m = 1);
				if (!l.skipFinalValidation && !y(b)) return e.push(b),
				1
			}
		...
		function l(a, b, c) {
			...
			if (B.$empty[d] || !a.children.length) "hr" == d && "br" == b ? a.replaceWith(w()) : (a.parent && c.push({
				...
				if ("br" == b) a.previous && !u(a.previous) && (b = w(), b.insertBefore(a)), a.next && !u(a.next) && (b = w(), b.insertAfter(a)), a.replaceWithChildren();
				else {
					var d = a.children,
						e;
					b: {
						e = B[b];
						for (var l = 0, p = d.length, m; l < p; ++l)
							if (m = d[l], m.type == CKEDITOR.NODE_ELEMENT && !e[m.name]) {
								e = !1;
								break b
							} e = !0
					}
		...
		function m(a, b) {
			...
			for (l in d)
				if ("class" == l)
					for (c = d[l].split(/\s+/), m = a.classes.join("|"); r = c.pop();) {
		...
		CKEDITOR.filter.prototype = {
			...
			applyTo: function(a, b, c, d) {
						...
						a: {
							...
							if (m)
								for (f = 0; f < m.length; ++f)
									if ((E = d.match(m[f])) && E[0].length == d.length) {
										c = !0;
										break a
				...
				for (; f = a.pop();)
					if (c = f.el, c.parent) switch (E = B[c.parent.name] || B.span, f.check) {
						case "it":
			...
			O = CKEDITOR.filter.transformationsTools = {
				...
				transform: function(a, b) {
					if ("string" == typeof b) a.name = b;
						...
						for (l in e)
							if ("class" == l)
								for (c = a.classes.join("|"), m = e[l].split(/\s+/); r = m.pop();) - 1 == c.indexOf(r) && a.classes.push(r);
				...
				h = function(b) {
					a.call(this, b, function(a) {
						...
						for (g in a) {
							...
							if (!d[g]) {
								if (k.icons)
									for (var v = k.icons.split(","), C = v.length; C--;) CKEDITOR.skin.addIcon(v[C], k.path + "icons/" + (CKEDITOR.env.hidpi && k.hidpi ? "hidpi/" : "") + v[C] + ".png");
								d[g] = 1
							}
							if (A)
								for (A.split && (A = A.split(",")), k = 0; k < A.length; k++) f[A[k]] || b.push(A[k])
						}
		...
		function z(a) {
			...
			CKEDITOR.plugins.load(c.split(","), function(c) {
				...
				for (var f in c) {
					...
					if (k && (x = k.match(p)))
						for (; k = x.pop();) CKEDITOR.error("editor-plugin-required", {
							plugin: k.replace(",", ""),
							requiredBy: f
						});
				...
				CKEDITOR.scriptLoader.load(l, function() {
					for (var c = ["beforeInit", "init", "afterInit"], l = 0; l < c.length; l++)
		...
		CKEDITOR.htmlParser.prototype = {
			...
			parse: function(b) {
				for (var c, e, f = 0, h; c = this._.htmlPartsRegex.exec(b);) {
					e = c.index;
					if (e > f)
						if (f = b.substring(f, e), h) h.push(f);
						else this.onText(f);
		...
		CKEDITOR.htmlParser.fragment.fromHtml =
			function(f, h, k) {
				...
				var C = new CKEDITOR.htmlParser,
					n = h instanceof CKEDITOR.htmlParser.element ? h : "string" == typeof h ? new CKEDITOR.htmlParser.element(h) : new CKEDITOR.htmlParser.fragment,
					y = [],
					F = [],
					w = n,
					u = "textarea" == n.name,
				...
				C.onTagOpen = function(e, m, f, r) {
					...
					if (a(m)) y.push(m);
						...
						if ("br" == e) F.push(m);
							...
							for (; !(r = (f = w.name) ? CKEDITOR.dtd[f] || (w._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : c, m.isUnknown || w.isUnknown || r[e]);)
								if (w.isOptionalClose) C.onTagClose(f);
								else if (e in b && f in b) f = w.children, (f = f[f.length - 1]) && "li" == f.name || z(f = new CKEDITOR.htmlParser.element("li"), w), !m.returnPoint && (m.returnPoint = w), w = f;
							else if (e in CKEDITOR.dtd.$listItem &&
								!v(e, f)) C.onTagOpen("li" == e ? "ul" : "dl", {}, 0, 1);
							else if (f in d && !v(e, f)) !m.returnPoint && (m.returnPoint = w), w = w.parent;
							else if (f in CKEDITOR.dtd.$inline && y.unshift(w), w.parent) z(w, w.parent, 1);
							else {
								m.isOrphan = 1;
								break
							}
				...
				C.onText = function(a) {
					...
					if (!u && !f["#"] && b in d) C.onTagOpen(e[b] || ""), C.onText(a);
					else {
						t();
						q();
						l || u || (a = a.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, " "));
		...
		CKEDITOR.htmlParser.filter = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				onElement: function(a, c) {
					for (var d = [this.elementsRules["^"], this.elementsRules[c.name], this.elementsRules.$], f, h = 0; 3 > h; h++)
						if (f = d[h]) {
							f = f.exec(a, c, this);
							if (!1 === f) return null;
		...
		a.prototype = {
			...
			exec: function(a, c) {
				var d = c instanceof CKEDITOR.htmlParser.node || c instanceof CKEDITOR.htmlParser.fragment,
					...
					q, t, g, z;
				for (z = 0; z < k; z++)
					if (d && (q = c.type, t = c.name), g = h[z], !(a.nonEditable && !g.options.applyToAll ||
				...
				return function(e) {
					if (e.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						var l = [],
							...
							u, E;
						if (p)
							for (r(p, 1) && l.push(p); p;) f(p) && (u = c(p)) && r(u) && ((E = c(u)) && !f(E) ? l.push(u) : (m(g).insertAfter(u), u.remove())), p = p.previous;
	...
	(function() {
		...
		CKEDITOR.inlineAll = function() {
			...
			for (b in CKEDITOR.dtd.$editable)
				for (var c = CKEDITOR.document.getElementsByTag(b), e = 0, f = c.count(); e < f; e++) a = c.getItem(e), "true" == a.getAttribute("contenteditable") && (d = {
					element: a,
					config: {}
	...
	(function() {
		function a(a) {
			...
			if (CKEDITOR.env.gecko || CKEDITOR.env.ie && CKEDITOR.env.needsBrFiller)
				if (g = d(g, e)) g.appendBogus(), f = CKEDITOR.env.ie;
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				insertElementIntoRange: function(a, b) {
					...
					if (r)
						for (;
							(g = b.getCommonAncestor(0, 1)) && (f = CKEDITOR.dtd[g.getName()]) &&
		...
		var g = CKEDITOR.dom.walker.whitespaces(!0),
			...
			n = function() {
				...
				function b(c, d) {
					...
					for (var h = B[h.getName()], r = 0, D = c.getChildren(), k = D.count(), G = -1, n = -1, t = 0, J = e.contains(B.$list); r <
						...
						node: e,
						allowed: 1
					}); - 1 < G && (f[G].firstNotAllowed = 1); - 1 < n && (f[n].lastNotAllowed = 1);
					return f
				}
				...
				function G(b, c) {
					...
					var e = c.endContainer.getChild(c.endOffset),
						l = c.endContainer.getChild(c.endOffset - 1);
					e && d(e, b[b.length - 1]);
				...
				return function(n, x, H, I) {
					...
					if (!I.checkReadOnly()) {
						...
						for (;
							(v = e(x)) && a(v) && v.isBlockBoundary() && K.contains(v);) x.moveToPosition(v, CKEDITOR.POSITION_BEFORE_END);
						...
						if (L = H) {
							...
							if ((A = L.startPath().block) && (1 != A.getChildCount() || !A.getBogus())) a: {
								...
								if (1 == v.getChildCount() && a(z = v.getFirst()) && z.is(t) && !z.hasAttribute("contenteditable")) {
									...
									L = 0;
									for (I = A.count(); L < I; L++)
										if (x = A.getItem(L), !x.is(q)) break a;
								...
								if (w) H = 0 < I;
									...
									if (E && J.isElement) {
										...
										if (w) R && (X = z.splitElement(R),
											n.zombies.push(X), n.zombies.push(R));
										else {
											R = v.getName();
											Y = !I;
											w = I == x.length - 1;
				...
				eol: {
					detect: function(a, b) {
						var c = a.range,
							...
							f = new CKEDITOR.dom.elementPath(c.endContainer,
								b);
						d.collapse(1);
						e.collapse();
			...
			w = function() {
				...
				return {
					list: {
						...
						merge: function(a, c) {
							if (a.mergeListBookmark) {
								...
								if (a.mergeList) {
									var p = g.contains(CKEDITOR.dtd.$list),
					...
					table: function() {
						function a(c) {
							var e = [],
								g, l = new CKEDITOR.dom.walker(c),
								m = c.startPath().contains(d),
								f = c.endPath().contains(d),
								p = {};
							l.guard = function(a,
								l) {
								if (a.type == CKEDITOR.NODE_ELEMENT) {
								...
								if (l && m && a.equals(m)) g = c.clone(), g.setEndAt(m, CKEDITOR.POSITION_BEFORE_END), e.push(g);
								else if (!l && f && a.equals(f)) g = c.clone(), g.setStartAt(f, CKEDITOR.POSITION_AFTER_START), e.push(g);
								else {
						...
						return {
							...
							detectRanges: function(e, g) {
								var l = c(g, e.bookmark),
									f = l.clone(),
									h, k, x = l.getCommonAncestor();
	...
	(function() {
		...
		var v, C, n = CKEDITOR.dom.walker.invisible(1),
			y = function() {
				function a(b) {
					return function(a) {
		...
		var F = "function" != typeof window.getSelection,
			w = 1;
		...
		CKEDITOR.dom.selection.prototype = {
				...
				var a = F ? function() {
					...
					var b = function(b, c) {
						...
						for (var e = d.children, g, f, l = b.duplicate(), h = 0,
								x = e.length - 1, k = -1, m, n; h <= x;)
							if (k = Math.floor((h + x) / 2), g = e[k], l.moveToElementText(g), m = l.compareEndPoints("StartToStart", b), 0 < m) x = k - 1;
							else if (0 > m) h = k + 1;
						else return {
							container: d,
							offset: a(g)
						...
						return {
							container: n,
							offset: 0 < m ? -l : n.nodeValue.length + l
						}
					};
					return function() {
			...
			getSelectedText: function() {
				...
				var b = this.getNative(),
					b = F ? "Control" == b.type ? "" : b.createRange().text : b.toString();
							...
							if (F) {
								...
								for (v = 0; v < a.length; v++) {
									...
									try {
										...
										if (0 <= H.toString().indexOf("NS_ERROR_ILLEGAL_VALUE")) f.collapse(1), q.setEnd(f.endContainer.$, f.endOffset);
										else throw H;
									}
									g.addRange(q)
								}
	...
	(function() {
		...
		function b(c) {
			var g = c.document;
			if (c.collapsed) g = F(this, g), c.insertNode(g), c.moveToPosition(g, CKEDITOR.POSITION_BEFORE_END);
			else {
				var f = this.element,
					h = this._.definition,
				...
				for (r.getPosition(q) == CKEDITOR.POSITION_FOLLOWING && (r = 0); r;) {
					k = !1;
					if (r.equals(q)) r = null, k = !0;
					else {
						var w = r.type == CKEDITOR.NODE_ELEMENT ? r.getName() : null,
							u = w && "false" == r.getAttribute("contentEditable"),
							z = w &&
						...
						if (u && n && CKEDITOR.dtd.$block[w])
							for (var A = r, y = e(A), B = void 0, D = y.length, G = 0, A = D && new CKEDITOR.dom.range(A.getDocument()); G < D; ++G) {
								var B = y[G],
						...
						if (y)
							if (B = r.getParent(), y = h, D = f, G = l, !B || !(B.getDtd() || CKEDITOR.dtd.span)[D] && !G || y.parentRule && !y.parentRule(B)) k = !0;
							else {
								if (v ||
									...
									for (var w = r, Z;
										(k = !w.getNext(J)) && (Z = w.getParent(), m[Z.getName()]) && d(Z, t, h, S);) w = Z;
									v.setEndAfter(w)
								}
							}
						else k = !0;
		...
		function c(a) {
			...
			var c =
				...
				d = c.startNode;
			if (a.collapsed) {
				for (var e = new CKEDITOR.dom.elementPath(d.getParent(), a.root), g, f = 0, h; f < e.elements.length && (h = e.elements[f]) && h != e.block && h != e.blockLimit; f++)
		...
		function v(a, b) {
			var c = this._.definition,
				...
				f;
			for (f in d)
				if ("class" != f && !this._.definition.fullMatch || a.getAttribute(f) == p(f, d[f])) b && "data-" == f.slice(0, 5) || (g = a.hasAttribute(f), a.removeAttribute(f));
		...
		function C(a) {
			...
			for (var g in b)
				if (g != this.element)
					for (c = a.getElementsByTag(g), e = c.count() - 1; 0 <= e; e--) d = c.getItem(e), d.isReadOnly() || n(d, b[g])
		}
						...
						for (var h in c)
							if ("_length" != h) {
								...
								if ("style" == h) a: {
									...
									"string" == typeof e && (e = CKEDITOR.tools.parseCssText(e, !0));f = void 0;
									for (f in d)
										if (!(f in e) || e[f] != d[f] && "inherit" != d[f] && "inherit" != e[f]) {
											d = !1;
											break a
										} d = !0
								}
	...
	(function() {
		function a(a, c, d) {
			var f = a.serializable,
				...
				q = f ? c.document.getById(a.startNode) : a.startNode;
		...
		function y(b) {
			...
			function l(a, d) {
				...
				if (!c.getById("cke_pastebin")) {
					...
					var m = new CKEDITOR.dom.element(!CKEDITOR.env.webkit &&
						!e.is("body") || CKEDITOR.env.ie ? "div" : "body", c);
					m.setAttributes({
						id: "cke_pastebin",
						"data-cke-temp": "1"
					});
			...
			init: function(b) {
				...
				b.on("paste", function(a) {
					...
					if (!a.data.dataValue) {
						...
						else if (g = c.getData("text/plain")) a.data.dataValue =
			...
			var v = CKEDITOR.tools.addFunction(l),
				w = CKEDITOR.tools.addFunction(function(c, e) {
					...
					switch (e.getKeystroke()) {
						case b ? 39:
							37: case 9: return (b = CKEDITOR.document.getById(g + (c + 1))) || (b = CKEDITOR.document.getById(g +
								"0")), b.focus(), !1;
						case b ? 37:
							39: case CKEDITOR.SHIFT + 9: return (b = CKEDITOR.document.getById(g + (c - 1))) || (b = CKEDITOR.document.getById(g + (k.list.length - 1))), b.focus(), !1;
						case 27:
							return a.focus(), !1;
						case 13:
						case 32:
		...
		function q(a, c) {
			...
			if ("function" != typeof c || !1 !== c.call(this._.filebrowserSe))
				if (!e || !1 !== e.call(this._.filebrowserSe, a, c))
					if ("string" == typeof c && c && alert(c), a && (b = this._.filebrowserSe, d = b.getDialog(),
		...
		CKEDITOR.on("dialogDefinition", function(a) {
			if (a.editor.plugins.filebrowser)
				for (var c = a.data.definition, d, b = 0; b < c.contents.length; ++b)
					if (d = c.contents[b]) l(a.editor, a.data.name, c, d.elements),
	...
	CKEDITOR.plugins.add("scayt", {
		...
		bindEvents: function(a) {
			...
			a.on("beforeCommandExec", function(b) {
				...
				} else if ("bold" === b.data.name || "italic" === b.data.name || "underline" === b.data.name || "strike" === b.data.name || "subscript" === b.data.name || "superscript" === b.data.name || "enter" === b.data.name)
					if (c = a.scayt) c.removeMarkupInSelectionNode(), setTimeout(function() {
		...
		parseConfig: function(a) {
			...
			if (a.config.scayt_disableOptionsStorage) {
				var c = CKEDITOR.tools.isArray(a.config.scayt_disableOptionsStorage) ? a.config.scayt_disableOptionsStorage : "string" === typeof a.config.scayt_disableOptionsStorage ? [a.config.scayt_disableOptionsStorage] : void 0,
					...
					e = ["lang", "ignore-all-caps-words", "ignore-domain-names", "ignore-words-with-mixed-cases", "ignore-words-with-numbers"],
		...
		menuGenerator: function(a, c) {
			var d = a.scayt,
				...
				f = {},
				e = a.config.scayt_contextCommands.split("|");
			...
			for (var g in b)
				if (-1 != CKEDITOR.tools.indexOf(e, g.replace("scayt_", "")) || "all" == a.config.scayt_contextCommands) f[g] = CKEDITOR.TRISTATE_OFF, "function" !== typeof b[g].verification || b[g].verification(a) || delete f[g], a.addCommand(g, {
					exec: b[g].exec
				}), a.addMenuItem(g, {
			...
			a.checkDirty = CKEDITOR.tools.override(a.checkDirty, function(a) {
				return function() {
					...
					if (CKEDITOR.plugins.scayt && CKEDITOR.plugins.scayt.state[this.name] && this.scayt) {
						if (b = "ready" == this.status) var e = c.removeMarkupFromString(this.getSnapshot()),
	...
	var js = {
		...
		commandName = "pbckcode";
	CKEDITOR.plugins.add("pbckcode", {
		icons: "pbckcode",
		lang: ["fr", "en"],
		init: function(a) {
			var b = this;
			void 0 === a.config.pbckcode && (a.config.pbckcode = {});
			a.settings = CKEDITOR.tools.extend({
				cls: "",
				modes: [
					["HTML", "html"],
					["CSS", "css"],
			...
			a.settings.js = normalizeJsUrl(a.settings.js);
			a.on("instanceReady", function() {
				CKEDITOR.document.appendStyleSheet(b.path + "dialogs/style.css")
			});
			a.ui.addButton("pbckcode", {
				label: a.lang.pbckcode.addCode,
				command: commandName,
				toolbar: "pbckcode"
			});
			a.addCommand(commandName, new CKEDITOR.dialogCommand("pbckcodeDialog", {
				allowedContent: "pre[*]{*}(*)"
			}));
			a.getCommand(commandName).disable();
			CKEDITOR.dialog.add("pbckcodeDialog", b.path + "dialogs/pbckcode.js");
			a.contextMenu && (a.addMenuGroup("pbckcodeGroup"), a.addMenuItem("pbckcodeItem", {
				label: a.lang.pbckcode.editCode,
				icon: b.path + "icons/pbckcode.png",
				command: commandName,
				group: "pbckcodeGroup"
			}), a.contextMenu.addListener(function(a) {
				if (a.getAscendant("pre",
						true)) return {
					pbckcodeItem: CKEDITOR.TRISTATE_OFF
				}
			}));
			var c = [getScriptUrl(a.settings.js, js.ace), js.pbSyntaxHighlighter];
			CKEDITOR.scriptLoader.load(c, function() {
				a.getCommand(commandName).enable();
				CKEDITOR.scriptLoader.load([getScriptUrl(a.settings.js, js.aceExtWhitespace)])
			})
		}
	});

	function normalizeJsUrl(a) {
		return a.concat("/").replace(RegExp("([^:]/)/+", "g"), "$1")
	}

	function getScriptUrl(a, b) {
		return a + b
	};
